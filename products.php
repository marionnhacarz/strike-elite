...
<div class="container">
  <div class="page-heading">
    ...
    <?php if ($q): ?><p style="color:var(--text-dim)">Search results for "<?php echo h($q) ?>"</p><?php endif; ?>
  </div>

  <div class="main-nav" style="margin-bottom:24px;">
    <a href="products.php" class="<?php echo $category === '' ? 'active' : '' ?>">All</a>
    <?php foreach ($categories as $c): ?>
      <a href="products.php?category=<?php echo urlencode($c['slug']) ?>" class="<?php echo $category === $c['slug'] ? 'active' : '' ?>"><?php echo h($c['name']) ?></a>
    <?php endforeach; ?>
  </div>
