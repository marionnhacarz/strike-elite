...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($paymentMethod, $validPayments)) $errors[] = 'Please select a valid payment method.';
  ...
  <div class="about-section" style="align-items:flex-start;">
    <div class="form-page" style="margin:0; max-width:100%;">
      ...
      <form method="POST">
        ...
        <div class="field">
          ...
          <input type="text" name="shipping_name" value="<?= h($_POST['shipping_name'] ?? current_user()['username']) ?>" required>
        </div>
        <div class="field">
          <label>Shipping Address</label>
          <textarea name="shipping_address" required><?= h($_POST['shipping_address'] ?? '') ?></textarea>
        </div>
        ...
        <button type="submit" class="btn btn-primary btn-block">Place Order — <?= price($total) ?></button>
      </form>
    </div>
    ...
    <div class="cart-summary" style="margin:0;">
      ...
      <?php foreach ($items as $row): ?>
        <div class="row"><span><?= h($row['product']['name']) ?> ×<?= $row['qty'] ?></span><span><?= price($row['subtotal']) ?></span></div>
      <?php endforeach; ?>
